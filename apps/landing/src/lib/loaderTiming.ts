 ...
 *   （字符四组依次升起、进度条 99% 边界在 criticalReady 前不可越过）
 */
...
/**
 * 进度条阻尼系数：critical 就绪后加速收口（0.18），未就绪时缓慢追踪（0.075），
 * 避免 preload 进度跳变导致进度条视觉抖动。
 */
export function progressDampFactor(criticalReady: boolean): number {
  return criticalReady ? 0.18 : 0.075
}
...
export function stepDisplayedProgress(
  ...
  return displayed + (target - displayed) * progressDampFactor(criticalReady)
}

/**
 * 进度显示值钳制：critical 未就绪时上限 99（floor，不许假装完成）；
 * 就绪后向上取整并封顶 100（让收口动画能真正到达 100）。
 */
export function displayedProgressValue(displayed: number, criticalReady: boolean): number {
