...
export interface DecryptRevealInstance {
  ...
  resize: () => void;
  /** Pause animation work without releasing GPU resources. */
  pause: () => void;
  /** Resume animation work after a lifecycle pause. */
  resume: () => void;
...
export function createDecryptReveal(
  ...
  function frame(now: number) {
    if (destroyed || externallyPaused) return;
    if (!visible) {
      running = false;
      return;
    }
  ...
  function start() {
    if (destroyed || externallyPaused || running || !visible) return;
    running = true;
