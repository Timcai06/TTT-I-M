...
export interface ParticleScrollInstance {
  /** Local integration: map page progress onto the upstream scroll container. */
  setScrollState: (progress: number, delta: number) => void;
...
export function createParticleScroll(
  ...
  function syncCanvasSize() {
    const dpr = Math.min(window.devicePixelRatio || 1, Math.max(config.dprMax, 1));
