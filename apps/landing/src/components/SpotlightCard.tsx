 ...
 *   高光静止），并改用 `onPointerMove`（一次绑定覆盖鼠标/触控笔）。用 `e.currentTarget`
 *   读宿主元素，省掉 ref —— 多态壳组件转发 ref 会触发 react-hooks/refs。
...
export default function SpotlightCard({ as: Tag = 'div', children, className = '', ...rest }: SpotlightCardProps) {
  ...
  return createElement(
    Tag,
    { onPointerMove: handlePointerMove, className: `spotlight-card ${className}`.trim(), ...rest },
    children
  )
}
