  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      cancelHeroArrived = onChapterArrived((id) => {
        ...
        gsap.set('.hero__signature-stroke', { strokeDashoffset: 0 })
      ...
      // ── 签名出笔：句点落地的瞬间顺势甩出，一笔不断写到「蔡」收尾。
      // power1.inOut 让 swash 段提速、入字后行笔沉稳，像真实签名的节奏。 ──
      ...
      if (signatureStroke) {
        ...
        gsap.set(signatureStroke, { strokeDasharray: strokeLength, strokeDashoffset: strokeLength })
        tl.to(signatureStroke, { strokeDashoffset: 0, duration: 1.7, ease: 'power1.inOut' }, '-=1.6')
      }
