...
export function createArchiveLighting(renderer: WebGLRenderer, scene: Scene) {
  renderer.toneMapping = AgXToneMapping; renderer.toneMappingExposure = 1.12
  ...
  scene.environment = environment.texture; scene.environmentIntensity = .23
  const fill = new HemisphereLight('#dbe5ef', '#493422', .38)
  const window = new DirectionalLight('#dde9f4', 2.1)
  window.position.set(-3, 3.3, .5); window.target.position.set(0, .85, -.8)
  ...
  const task = new SpotLight('#ffd29c', 2.4, 2.5, .85, 1, 2)
  ...
  const shelf = new PointLight('#f6c58e', .12, 1.5, 2); shelf.position.set(.3, 2.37, -1.31)
