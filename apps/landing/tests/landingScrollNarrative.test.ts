...
void test('derives progress fills, chapter blend, and theme mix from one rect snapshot', () => {
  ...
  assert.equal(state.theme.bg.toLowerCase(), '#000000')
  assert.equal(getChapterTheme('about').bg.toLowerCase(), '#000000')
  assert.equal(getChapterTheme('frame').bg.toLowerCase(), '#000000')
