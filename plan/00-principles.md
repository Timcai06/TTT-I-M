...
- ✅ 保留：激进整站预热的**意图**。
- 🔧 修复：① 单图失败导致永久黑屏的缺陷（见 02 · A1）——已实现；
  ② 把「门控 intro 的部分（critical）」与「intro 退场后继续后台拉满的部分
  （deferred）」分开——总下载量不变，只缩小黑屏闸门。
  **已实现（2026-06-10）**：`preloadController` 暴露 `criticalReady` 作退场闸门，
  deferred 并发队列在退场后继续拉满到 `ready`；Frame DOM 图片保持 eager 兜底；
  `loader-preload-guards.mjs` 锁双向契约（不许回退全量门控，也不许砍后台拉满）。
