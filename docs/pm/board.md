...
> **2026-09-10 起，工作方式已改变。** tim 直接与单个会话协作迭代，不再经 PM 派卡。
> 本文件下方的阶段记录仍然准确地描述了 NR/VR 各卡当时的结论，但**不再描述当前状态**。
> 特别是「没有提交、推送、部署」这句已经失效：此后的成果已合并 `main` 并部署到生产
> `https://www.crt-dsg.com`。新会话不要据此认为工作区未提交或生产未更新，请以 `git log`
> 与实际部署产物为准。

## 直接协作期的已交付改动（2026-09-10 之后）

按主题归纳，细节见各次提交信息：

- **生产可达性**：CSP 曾分别挡掉内联字体、GLB 的 blob 纹理与 blob 影音；预加载门曾在慢网络下
  永不放行。现已按「必需 / 预热」分类降级，critical 任务自动重试。
- **加载体验**：进度按清单权重而非任务数计算，房间模型流式上报字节；图片预热 90s，重型预热
  600s 上限仅用于兜住挂死。
- **相机与叙事**：交接抬升由实测水平距离推导，对齐窗口按法线夹角展开，平摊面上的 lookAt
  奇点已修复；章节交接改为舞台吸顶后才开始；About 页由收缩改为原地溶解。
- **主题与层叠**：`personal-archive.css` 的重复引入已移除（它曾让 `natural-room.css` 的多条
  重置失效）；Frame 的 `.ag-panel`、Working set 的渐隐色改为跟随房间表面。
- **阅读对比度**：纸色校准增加对比度下限钳制，派生前景 token 按对比度目标重定。

## 历史阶段记录（下方内容为当时结论，非当前状态）
