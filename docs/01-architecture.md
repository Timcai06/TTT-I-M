...
- **Fixed (2026-06-06)**: `/_next/:path*` is rewritten to the studio origin and MUST stay the *first* rewrite entry (Vercel evaluates rewrites in order; a page rewrite before it would shadow static assets). `platform-guards.mjs` asserts both the entry and its position. Remaining risk: the guard is a source-string assertion — `tests/runtime/cross-zone-smoke.mjs` is the runtime check (main-domain `/blog` HTML + its `/_next` assets must all 200).
- studio's MDX is real MDX (2026-06-06): `MdxContent.tsx` compiles via `next-mdx-remote/rsc` on the server (compiler stays out of the client bundle), frontmatter via `gray-matter`. The platform guard pins this so it can't regress to the old hand-rolled markdown subset.
