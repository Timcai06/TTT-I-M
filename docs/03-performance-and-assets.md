...
- **Why it blocks**: `criticalReady` is only the SYSTEM → ARCHIVE phase marker. The intro exits on `renderReady || readingFallbackReady` — the whole manifest succeeding, or every required task succeeding while only flagged prewarm failed. It is never `renderReady` alone; treating it that way is what once let a slow connection strand the intro forever.
...
- **Preload tiers**: critical tasks run first, then the bounded visual set runs eight-wide. Exit is `renderReady || readingFallbackReady`, never `renderReady` alone.
  Deadlines are per class, not one number: 12s stays the generic network default, image prewarm gets 90s because it shares bandwidth with the 22.9 MB room, and the three heavy prewarms (room, chapter pages, film/sound) get a 600s ceiling that exists only to break a stalled socket. A shorter ceiling turns "slow" into "failed" and silently costs the visitor the room.
  Manifest tasks carry a `weight` in units of one average preloaded image, and may expose `progress()` for live sub-task reporting; the room streams its bytes through it. Dividing by task count instead put ~55 quick image fetches and one 22.9 MB model on the same footing, which sent the bar to the nineties and then stopped it dead.
  Critical tasks retry up to three times with backoff before being recorded as failed, so a transient hiccup does not hand the reader a retry panel that a plain refresh would have cleared.
  Every task receives a child `AbortSignal`: its deadline and Loader unmount both cancel image requests, remove queued decodes, detach global activity listeners and prevent late state writes. Dynamic imports/font readiness cannot be physically cancelled by browser APIs, but the controller stops awaiting them immediately and ignores their later completion. Liquid Metal and the Stack → Work Spark document are consumed into the HTTP/source cache before hand-off. A failed resource remains a recorded non-fatal skip, so one bad URL cannot strand the intro. A lightweight `window.__portfolioPreloadDebug.snapshot()` remains readable in production for deterministic support/E2E evidence; only development builds allocate stall timers or write console reports.
